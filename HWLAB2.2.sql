...
SELECT TenDuAn, NgayKetThuc FROM dbo.DuAn
...
SELECT ChuyenNganh, COUNT(*) AS N'Số lượng chuyên gia'
FROM dbo.ChuyenGia
GROUP BY ChuyenNganh

-- Tìm chuyên gia có số năm kinh nghiệm cao nhất.
SELECT * FROM dbo.ChuyenGia
WHERE NamKinhNghiem = (SELECT MAX(NamKinhNghiem) FROM dbo.ChuyenGia
...
FROM dbo.DuAn JOIN dbo.CongTy ON CongTy.MaCongTy = DuAn. MaCongTy
WHERE DuAn. TrangThai = N'Dang thuc hiên'
